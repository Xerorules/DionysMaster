...
namespace CAPA_DATOS
{
    public class D_VENTA
    {
        ...
        public DataTable LISTAR_TIPO_MOVIMIENTO()
        {
            ...
            cmd.CommandText = "SELECT ID_TIPOMOV,DESCRIPCION  FROM TIPO_MOVIMIENTO WHERE ID_TIPOMOV IN ('EGE','EVA','IVA')";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con;
        ...
        public DataTable CONSULTA_IMPRESION_CAJA_KARDEX(string ID_CAJA)
        {
            ...
            cmd.CommandText = @"SELECT ID_MOVIMIENTO,DESCRIPCION,(SELECT SERIE +'-'+ NUMERO FROM VENTA WHERE ID_VENTA = ID_COMPVENT) AS NUMERO,(SELECT TIPO_DOC FROM VENTA WHERE ID_VENTA = ID_COMPVENT) AS TIPO_DOCU,
                                ID_COMPVENT,ID_TIPOPAGO,TP_DESCRIPCION,ID_TIPOMOV,TM_DESCRIPCION,IMPORTE,MONEDA,TIPO_CAMBIO,AMORTIZADO,ID_CAJA,
